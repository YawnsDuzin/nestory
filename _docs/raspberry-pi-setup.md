...
### LAN-only 규칙을 any 로 전환 (공유기 포트포워딩 사용 시)

위에서 LAN-only(`from 192.168.0.0/24`)로 허용했는데 이후 공유기 포트포워딩(예: 외부 9082 → 내부 8082)으로 외부 노출까지 필요해진 경우, 외부 클라이언트의 source IP는 LAN 대역이 아니므로 기존 규칙이 패킷을 차단합니다(타임아웃). 기존 LAN-only 규칙을 지우고 any 로 다시 추가:
...
> 공유기는 DNAT으로 destination만 내부 IP로 바꿀 뿐 source IP는 외부 클라이언트의 공인 IP 그대로 전달합니다. 따라서 `from 192.168.0.0/24` 매칭이 실패. 공유기 자체 IP를 source로 화이트리스트하는 우회도 동작하지 않습니다.

### 공유기 포트포워딩 (iptime 기준 — 외부 접속)

외부 인터넷에서 라즈베리파이로 접속하려면 공유기에서 **외부 포트 → 내부 IP:내부 포트** 매핑이 필요합니다. 외부/내부 포트를 분리하면 동일 내부 IP에 여러 서비스를 운영할 때 충돌을 피할 수 있고(예: 8000·8082 동시 실행), 잘 알려진 포트 스캔 표적도 회피할 수 있어 권장됩니다.

**iptime A3004NS-M 기준 메뉴 경로**:

```
관리도구 → 고급 설정 → NAT/라우터 관리 → 포트포워드 설정
```

**규칙 예시** (외부 9082 → 내부 192.168.0.231:8082):

| 항목 | 값 |
| --- | --- |
| 규칙이름 | `nestory` |
| 내부 IP주소 | `192.168.0.231` (라즈베리파이 LAN IP — `hostname -I`로 확인) |
| 프로토콜 | `TCP` |
| 외부 포트 | `9082` |
| 내부 포트 | `8082` |

> "새규칙" → 위 표대로 입력 → **반드시 우상단 "저장" 버튼 클릭** (NVRAM 저장). 저장을 누르지 않으면 재부팅 시 설정이 휘발됩니다.

**WAN(외부) IP 확인** — 같은 iptime 관리도구에서:

```
고급 설정 → 네트워크 관리 → 인터넷 설정 정보 → 외부 IP 주소
```
...
**전제 조건** (모두 충족해야 외부에서 도달):

1. uvicorn이 `0.0.0.0:8082`에 바인딩 — `ss -tlnp | grep 8082` 출력에 `0.0.0.0:8082` 확인 (`127.0.0.1:8082`이면 외부 차단)
2. UFW가 비활성이거나 `8082/tcp ALLOW IN Anywhere` (앞 "LAN-only 규칙을 any 로 전환" 참고)
3. iptables 직접 사용 시 `INPUT` 체인에 `-p tcp --dport 8082 -j ACCEPT` (source 지정 없이) — `sudo iptables -L INPUT -n -v | grep 8082`로 확인

검증 — 외부 네트워크(휴대폰 LTE 등)에서:
...
- **타임아웃** — 위 전제 1·2·3 중 하나가 빠진 경우. 가장 흔한 케이스는 UFW가 LAN-only로 막혀 있는 것(앞 서브섹션 참고). LAN 내부에서 `http://192.168.0.231:8082/`는 되는데 외부 9082만 안 되면 100% Pi 측 방화벽 또는 source-IP 제한 문제.
- **이중 NAT** — ISP가 공유기 WAN에 사설 IP(10.x / 100.64.x / 172.16~31.x / 192.168.x)를 할당한 경우 외부 포트포워딩 자체가 불가능. 인터넷 설정 정보의 "외부 IP 주소"가 사설 대역이면 ISP에 공인 IP 요청 또는 Cloudflare Tunnel · Tailscale Funnel 같은 우회 사용.
- **ISP 포트 차단** — 일부 ISP가 80·443·25 등 well-known 포트, 그리고 일부 9000번대를 차단. 외부 포트를 다른 번호(예: 18082)로 바꿔 차이가 있는지 비교.
- **저장 버튼 미클릭** — iptime 일부 펌웨어는 규칙 추가 후 우상단 "저장"을 눌러야 영구 반영. 재부팅 후 규칙이 사라지면 이 케이스.
- **AP 모드** — iptime이 AP(브리지) 모드면 NAT 자체가 동작 안 함. 시스템 요약에서 "라우터 모드" 확인.
